...
{
    public class BbLMP : Basic.BasicBaseball
    {
        private string sWebUrl1 = UrlSetting.GetUrl(ESport.Baseball_LMP, "Url1");
        public BbLMP(DateTime today) : base(ESport.Baseball_LMP)
        {            
            // 設定
            this.AllianceID = 71;
            this.GameType = "BBMX";
            ...
            {
                this.sWebUrl = @"http://www.milb.com/lookup/json/named.schedule_vw_complete.bam?game_date='{0}'&season={1}&league_id=132";
            }
            ...
            string season = frmMain.bblmp_season;
            int tryParseInt;
            if (string.IsNullOrEmpty(season) || !int.TryParse(season, out tryParseInt))
                season = this.GameDate.ToString("yyyy");
